    ...
    if "example" in filename.lower():
        ...
        if len(matches) > 0:
            ...
            for match in matches:
                ...
                code_block_wrapped_text += text[prev_end: start]
                code_block_wrapped_text += MD_CODE_BLOCK_TITLE.format(title, lisp_code)
                prev_end = end
            code_block_wrapped_text += text[prev_end:]
        
        elif text.find(MD_CODE_BEGIN) == -1:
            code_block_wrapped_text = f'{MD_CODE_BEGIN}\n{text}\n{MD_CODE_END}'
        
        if len(code_block_wrapped_text) != 0:
...
def conditionally_add_section_codeblock(filename, root):
    ...
    text_w_code_blocks = "".join([apply_first_example_code_block(text_part, filename) for text_part in text_parts])
    file = open(filepath, "w")
